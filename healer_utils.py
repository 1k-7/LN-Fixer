        ...
        try:
            ...
            for i_tag in soup.find_all('i'):
                if 'Generated by' in i_tag.text and 'Lightnovel Crawler' in i_tag.text:
                    ...
                    if prev and prev.name == 'br': prev.decompose()
                    i_tag.decompose()
                    break
...
def extract_url_from_epub(epub_path):
    ...
    try:
        ...
        shutil.rmtree(extract_dir, ignore_errors=True)
        return None, extract_dir, "Bad Zip File (Likely 0 bytes)"
    except Exception as e:
        shutil.rmtree(extract_dir, ignore_errors=True)
        return None, extract_dir, f"Zip Crash: {str(e)}"
    ...
    if not match:
        shutil.rmtree(extract_dir, ignore_errors=True)
        return None, extract_dir, "Source URL not found"
...
def extract_anchor_number(title):
    ...
    vol_match = re.search(r'(?:volume|vol|v)\s*(\d+)', t)
    ch_match = re.search(r'(?:chapter|ch\.?|c)\s*(\d+(?:\.\d+)?)', t)
    
    vol = float(vol_match.group(1)) if vol_match else 0.0
    ch = float(ch_match.group(1)) if ch_match else None
...
def fix_epub_spine_fallback(epub_path, extract_dir, log_data):
    ...
    if toc_xhtml:
        ...
        if nav_list:
            ...
            with open(toc_xhtml, 'w', encoding='utf-8') as f: f.write(str(toc_soup))
    ...
    if is_order_changed:
        log_data.append(f"🧮 404 Fallback applied: Numerically sorted {len(epub_chapters)} chapters.")
        return "FIXED", fixed_epub_path
    else:
        log_data.append("✅ 404 Fallback check: EPUB order was already numerical. Removed watermark.")
...
def fix_epub_spine(epub_path, extract_dir, canonical_toc, log_data):
    ...
    if is_order_changed:
        log_data.append(f"🛠️ Successfully mapped {len(epub_chapters)} chapters 1:1 with source.")
        return "FIXED", fixed_epub_path
    else:
        log_data.append("✅ EPUB order was already perfectly synced. Removed watermark.")
        return "OK", fixed_epub_path
